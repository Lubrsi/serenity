...
UNMAP_AFTER_INIT void RTL8168NetworkAdapter::detect()
{
    static const PCI::ID rtl8168_id = { 0x10EC, 0x8168 };
    PCI::enumerate([&](const PCI::Address& address, PCI::ID id) {
        ...
        [[maybe_unused]] auto& unused = adopt_ref(*new RTL8168NetworkAdapter(address, irq)).leak_ref();
    });
}
...
void RTL8168NetworkAdapter::test()
{
    phy_out(PHY_REG_BMCR, phy_in(PHY_REG_BMCR) | BMCR_RESET);
    while ((phy_in(PHY_REG_BMCR) & BMCR_RESET) != 0)
        ;
...
void RTL8168NetworkAdapter::start_hardware(bool full)
{
...
void RTL8168NetworkAdapter::hardware_quirks_h()
{
    ...
    // vendor magic values ???
    eri_update(0xDc, ERI_MASK_0001, 0x00, 0x1, ERI_EXGMAC);
    eri_update(0xDc, ERI_MASK_0001, 0x1, 0x00, ERI_EXGMAC);
    eri_update(0xDc, ERI_MASK_1111, 0x10, 0x00, ERI_EXGMAC);
    eri_update(0xD4, ERI_MASK_1111, 0x1F00, 0x00, ERI_EXGMAC);
