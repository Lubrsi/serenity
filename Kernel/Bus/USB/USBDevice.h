...
public:
    static KResultOr<NonnullRefPtr<Device>> try_create(USBController const&, u8, Pipe::DeviceSpeed);

    Device(USBController const&, u8, NonnullOwnPtr<Pipe> default_pipe);
    ...
    Pipe const& default_pipe() const { return *m_default_pipe; }

protected:
    Device(NonnullRefPtr<USBController> controller, u8 address, u8 port, NonnullOwnPtr<Pipe> default_pipe);
