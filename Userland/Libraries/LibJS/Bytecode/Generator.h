...
public:
    ...
    static CodeGenerationErrorOr<NonnullOwnPtr<Executable>> generate(ASTNode const&, bool strict_mode, FunctionKind = FunctionKind::Normal);
    ...
    void perform_needed_unwinds(bool is_break_node = false) requires(OpType::IsTerminator)
    {
        ...
        for (size_t i = m_boundaries.size(); i > 0; --i) {
            ...
            else if (boundary == BlockBoundaryType::ClearPendingException)
                emit<Bytecode::Op::ClearPendingException>();
        }
    }
