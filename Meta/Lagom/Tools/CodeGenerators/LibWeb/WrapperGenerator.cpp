...
static OwnPtr<Interface> parse_interface(StringView filename, StringView const& input)
{
    ...
    auto parse_function = [&](HashMap<String, String>& extended_attributes, IsSpecialOperation is_special_operation = IsSpecialOperation::No) {
        bool static_ = false;
        if (is_special_operation == IsSpecialOperation::No && lexer.consume_specific("static")) {
            static_ = true;
            consume_whitespace();
        }
...
#include <LibJS/Runtime/Object.h>
    ...
    // Legacy platform objects define functions that take LibWeb objects, so LibWeb's Foward header is required here.
...
private:
    ...
    if (interface.is_legacy_platform_object()) {
        generator.append(R"~~~(
    bool is_named_property_exposed_on_object(@fully_qualified_name@&, JS::PropertyName const&) const;
...
void @prototype_class@::initialize(JS::GlobalObject& global_object)
{
    ...
    if (!interface.attributes.is_empty() || !interface.functions.is_empty() || interface.has_stringifier || interface.is_legacy_platform_object()) {
        generator.append(R"~~~(
...
JS_DEFINE_NATIVE_FUNCTION(@class_name@::to_string)
{
    ...
    if (interface.is_legacy_platform_object()) {
        auto scoped_generator = generator.fork();
        scoped_generator.set("class_name", interface.prototype_class);
...
static JS::Value wrap_for_legacy_platform_object_get_own_property([[maybe_unused]] JS::VM& vm, [[maybe_unused]] auto& retval)
{
        ...
        if (interface.supports_named_properties()) {
            ...
            scoped_generator.append(R"~~~(
bool @class_name@::is_named_property_exposed_on_object(@fully_qualified_name@& impl, JS::PropertyName const& property_name) const
{
    auto& vm = this->vm();
    ...
    bool has_own_property_named_p = has_own_property(property_name);
    if (vm.exception())
        return {};

    if (has_own_property_named_p)
        return false;
)~~~");
    ...
    return true;
        ...
        auto generate_legacy_platform_object_get_own_property_function = [&](IDL::Interface const& interface, IgnoreNamedProps ignore_named_props, String const& for_which_internal_method) {
...
Optional<JS::PropertyDescriptor> @class_name@::legacy_platform_object_get_own_property_for_@internal_method@_slot(JS::PropertyName const& property_name) const
{
            ...
            // NOTE: The spec doesn't say when to do this and the impl is required almost immediately, e.g. for "supported property index".
            //       So, let's try getting the impl straight away.
            get_own_property_generator.append(R"~~~(
    auto& vm = this->vm();
    auto& global_object = this->global_object();

    dbgln("{}{}", vm.this_value(global_object).to_string_without_side_effects(), property_name.as_string());
    ...
    if (IDL::is_an_array_index(global_object, property_name)) {
        ...
        // NOTE: This is in it's own variable to enforce the type.
        // FIXME: Can this throw?
        Vector<u32> supported_property_indices = impl->support_property_indices();
        if (supported_property_indices.contains_slow(index)) {
)~~~");
            ...
            auto result = throw_dom_exception_if_needed(vm, global_object, [&] { return impl->determine_value_of_indexed_property(index); });
                ...
                // 4. Otherwise, operation was defined with an identifier. Set value to the result of performing the method steps of operation with O as this and « index » as the argument values.
                    ...
                    function_scoped_generator.set("function.cpp_name", interface.indexed_property_getter->name);
    ...
    if (is_named_property_exposed_on_object(*impl, property_name)) {
        ...
        auto result = throw_dom_exception_if_needed(vm, global_object, [&] { return impl->determine_value_of_named_property(property_name_string); });
                ...
                // 4. Otherwise, operation was defined with an identifier. Set value to the result of performing the method steps of operation with O as this and « index » as the argument values.
                    ...
                    auto function_scoped_generator = get_own_property_generator.fork();
                    function_scoped_generator.set("function.cpp_name", interface.named_property_getter->name);
    ...
    return Object::internal_get_own_property(property_name);
        ...
        generate_legacy_platform_object_get_own_property_function(interface, IgnoreNamedProps::No, "get_own_property");

        // Step 2 of [[Set]]: Let ownDesc be LegacyPlatformObjectGetOwnProperty(O, P, true).
        generate_legacy_platform_object_get_own_property_function(interface, IgnoreNamedProps::Yes, "set");
...
static void invoke_named_property_setter(JS::GlobalObject& global_object, @fully_qualified_name@& impl, String const& property_name, JS::Value value)
{
    ...
    [[maybe_unused]] bool creating = !supported_property_names.contains_slow(property_name);
            ...
            // 3. Let T be the type of the second argument of operation. (NOTE: Not necessary, T is value)
    ...
    if (creating) {
        ...
        if (should_return_empty(result))
            return;
                ...
                function_scoped_generator.set("function.cpp_name", interface.named_property_setter->name);
...
static void invoke_indexed_property_setter(JS::GlobalObject& global_object, @fully_qualified_name@& impl, JS::PropertyName const& property_name, JS::Value value)
{
    ...
    // FIXME: Can this throw?
    Vector<u32> supported_property_indices = impl->support_property_indices();
    [[maybe_unused]] bool creating = !supported_property_indices.contains_slow(index);
)~~~");

            // 3. Let operation be the operation used to declare the named property setter. (NOTE: Not necessary)
            // 4. Let T be the type of the second argument of operation. (NOTE: Not necessary, T is value)
    ...
    if (creating) {
        ...
        if (should_return_empty(result))
            return;
                ...
                auto function_scoped_generator = scoped_generator.fork();
                function_scoped_generator.set("function.cpp_name", interface.indexed_property_setter->name);
...
Optional<JS::PropertyDescriptor> @class_name@::internal_get_own_property(JS::PropertyName const& property_name) const
{
    ...
    return legacy_platform_object_get_own_property_for_get_own_property_slot(property_name);
        ...
        // NOTE: The spec doesn't say when to do this and the impl is required almost immediately, e.g. for "invoke the indexed property setter".
        //       So, let's try getting the impl straight away.
        scoped_generator.append(R"~~~(
...
bool @class_name@::internal_delete(JS::PropertyName const& property_name)
{
    ...
    if (is_named_property_exposed_on_object(*impl, property_name)) {
        ...
        if (!succeeded)
            return false;
                    ...
                    auto function_scoped_generator = scoped_generator.fork();
                    function_scoped_generator.set("function.cpp_name", interface.named_property_deleter->name);
    ...
    auto own_property_named_p_descriptor = internal_get_own_property(property_name);
    if (vm.exception())
        return {};
...
{
    dbgln("internal_prevent_extensions!");
    // 1. Return false.
    return false;
}
)~~~");

        // FIXME: 3.9.6. [[OwnPropertyKeys]], https://heycam.github.io/webidl/#legacy-platform-object-ownpropertykeys
        scoped_generator.append(R"~~~(
