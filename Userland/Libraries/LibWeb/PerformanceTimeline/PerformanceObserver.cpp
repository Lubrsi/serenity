...
{
    return MUST_OR_THROW_OOM(realm.heap().allocate<PerformanceObserver>(realm, realm, callback));
}
...
JS::ThrowCompletionOr<void> PerformanceObserver::initialize(JS::Realm& realm)
{
...
WebIDL::ExceptionOr<void> PerformanceObserver::observe(PerformanceObserverInit const& options)
{
    dbgln("=== observe");
    ...
    if (m_observer_type == ObserverType::Undefined) {
        ...
        if (options.entry_types.has_value()) {
            for (auto& type : options.entry_types.value()) {
                dbgln("observe multi type: {}", type);
            }
        ...
        if (options.type.has_value()) {
            dbgln("observe single type: {}", options.type.value());
...
#undef __ENUMERATE_SUPPORTED_PERFORMANCE_ENTRY_TYPES
        ...
        TRY_OR_THROW_OOM(vm, m_options_list.try_append(options));
        TRY_OR_THROW_OOM(vm, relevant_global->register_performance_observer(*this));
    }
        ...
        if (options.buffered.has_value() && options.buffered.value()) {
            ...
            for (auto const& entry : tuple.performance_entry_buffer) {
                ...
                if (entry->should_add_entry(options) == ShouldAddEntry::Yes)
                    TRY_OR_THROW_OOM(vm, m_observer_buffer.try_append(JS::make_handle(*entry)));
            }
...
{
    // FIXME: 1. Remove this from the list of registered performance observer objects of relevant global object.
...
ErrorOr<void> PerformanceObserver::append_to_observer_buffer(Badge<HTML::WindowOrWorkerGlobalScopeMixin>, JS::Handle<PerformanceTimeline::PerformanceEntry> entry)
{
    return m_observer_buffer.try_append(entry);
}
