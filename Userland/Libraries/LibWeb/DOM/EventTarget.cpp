...
void EventTarget::activate_event_handler(FlyString const& name, HTML::EventHandler& event_handler, IsAttribute is_attribute)
{
    ...
    //        For anything else, it came from JavaScript, so use the incumbent global object (the global object of the realm of the code that called this, but if this being done in a callback, it will be the global object of the code that caused the callback)
