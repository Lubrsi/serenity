...
Bindings::CallbackType* EventTarget::get_current_value_of_event_handler(FlyString const& name)
{
    ...
    if (event_handler.value.has<String>()) {
        ...
        auto* function = JS::ECMAScriptFunctionObject::create(global_object, name, program->body(), program->parameters(), program->function_length(), scope, nullptr, JS::FunctionKind::Normal, program->is_strict_mode(), program->might_need_arguments_object(), is_arrow_function);
        VERIFY(function);
