...
void queue_mutation_observer_microtask(DOM::Document& document)
{
    ...
    // NOTE: This uses the implied document concept. In the case of mutation observers, it is always done in an element context, so document should be that element's node document.
    ...
    HTML::queue_a_microtask(&document, [&custom_data]() {
        ...
        for (auto& mutation_observer : notify_set) {
            ...
            if (!records.is_empty()) {
                ...
                dbgln("Invoking Mutation Observer Callback!!!");
