...
ThrowCompletionOr<TemporalDuration> difference_zoned_date_time(GlobalObject& global_object, BigInt const& nanoseconds1, BigInt const& nanoseconds2, Value time_zone, Object& calendar, StringView largest_unit, Object* options)
{
    auto& vm = global_object.vm();
    ...
    // FIXME: Remove the as_object()!
    auto* intermediate = MUST(create_temporal_zoned_date_time(global_object, *intermediate_ns, time_zone.as_object(), calendar));
    ...
    auto time_difference = MUST(balance_duration(global_object, 0, 0, 0, 0, 0, 0, *result.nanoseconds.cell(), "hour"));
