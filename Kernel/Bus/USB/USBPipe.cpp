...
KResultOr<NonnullOwnPtr<Pipe>> Pipe::try_create_pipe(NonnullRefPtr<USBController> controller, Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, i8 device_address, u8 poll_interval)
{
...
Pipe::Pipe(NonnullRefPtr<USBController> controller, Type type, Pipe::Direction direction, u16 max_packet_size)
...
Pipe::Pipe(NonnullRefPtr<USBController> controller, Type type, Direction direction, USBEndpointDescriptor& endpoint [[maybe_unused]])
    : m_controller(controller)
    , m_type(type)
    , m_direction(direction)
{
...
Pipe::Pipe(NonnullRefPtr<USBController> controller, Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, u8 poll_interval, i8 device_address)
    : m_controller(controller)
    , m_type(type)
    , m_direction(direction)
