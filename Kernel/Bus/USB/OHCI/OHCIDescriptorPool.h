...
public:
    static OwnPtr<OHCIDescriptorPool<T>> try_create(const StringView name)
    {
        auto pool_memory_block = MM.allocate_kernel_region(PAGE_SIZE, "OHCI Descriptor Pool", Memory::Region::Access::ReadWrite);
        if (!pool_memory_block)
            return {};

        return adopt_own_if_nonnull(new (nothrow) OHCIDescriptorPool(pool_memory_block.release_nonnull(), name));
    }

    OHCIDescriptorPool(NonnullOwnPtr<Memory::Region> pool_memory_block, const StringView& name)
        : m_pool_name(name)
...
public:
    OHCIDescriptorPool() = delete;
    ~OHCIDescriptorPool() = default;
    ...
    [[nodiscard]] T* try_take_free_descriptor()
    {
        ...
        dbgln_if(OHCI_DEBUG, "Got a free OHCI Descriptor @ {} from pool {}", m_free_descriptor_stack.top(), m_pool_name);
    ...
    {
        dbgln_if(OHCI_DEBUG, "Returning descriptor @ {} to pool {}", ptr, m_pool_name);
