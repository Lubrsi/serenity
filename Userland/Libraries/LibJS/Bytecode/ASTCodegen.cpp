...
Bytecode::CodeGenerationErrorOr<void> LabelledStatement::generate_labelled_evaluation(Bytecode::Generator& generator, Vector<FlyString> const& label_set) const
{
    ...
    if (is<IterationStatement>(labelled_item)) {
        ...
        generator.start_boundary(Bytecode::Generator::BlockBoundaryType::BreakLabel);
        ...
        generator.end_boundary(Bytecode::Generator::BlockBoundaryType::BreakLabel);
...
Bytecode::CodeGenerationErrorOr<void> WhileStatement::generate_labelled_evaluation(Bytecode::Generator& generator, Vector<FlyString> const& label_set) const
{
    ...
    generator.start_boundary(Bytecode::Generator::BlockBoundaryType::ContinueLabel);
    generator.start_boundary(Bytecode::Generator::BlockBoundaryType::BreakLabel);
    ...
    generator.end_boundary(Bytecode::Generator::BlockBoundaryType::BreakLabel);
    generator.end_boundary(Bytecode::Generator::BlockBoundaryType::ContinueLabel);
...
Bytecode::CodeGenerationErrorOr<void> ForStatement::generate_labelled_evaluation(Bytecode::Generator& generator, Vector<FlyString> const& label_set) const
{
    ...
    Bytecode::BasicBlock* load_result_and_then_exit_block { nullptr };

    auto& body_block_ptr = generator.make_block();
    auto& end_block = generator.make_block();

    bool has_lexical_environment = false;

    // Since a labelled break will jump straight to the end block, we need a special `BreakLabel` boundary, as there may be boundaries above the `Break` boundary that may still need unwinding.
    // For example, the scope created in the `if (m_init)` block below, which adds a `LeaveLexicalEnvironment` boundary before the `Break` boundary.
    generator.start_boundary(Bytecode::Generator::BlockBoundaryType::BreakLabel);
    ...
    // Using a labelled continue will make us jump to the update block instead of the end block, meaning we have to keep the scope from above alive,
    // but everything else must be unwound.
    generator.start_boundary(Bytecode::Generator::BlockBoundaryType::ContinueLabel);

    if (m_test) {
        test_block_ptr = &generator.make_block();
        load_result_and_then_exit_block = &generator.make_block();
    } else {
    ...
    if (m_update)
        ...
        update_block_ptr = test_block_ptr;
    else
    ...
    if (m_test) {
        ...
        // Store the result of executing the body into the result register.
        // This is done here because the body block may have been block terminated (e.g. via unconditional `continue`), meaning we can't generate the store in the body block.
        // Note this is only done if there isn't an update block, as the body will jump to that first if it exists. If it doesn't, it will jump here first.
        // If neither the test block or update block exists, the accumulator will always store the result of the body.
        if (!m_update)
            generator.emit<Bytecode::Op::Store>(result_reg);
        ...
        generator.emit<Bytecode::Op::JumpConditional>().set_targets(
            ...
            Bytecode::Label { *load_result_and_then_exit_block });
    }
    ...
    // We do not need to load the result before jumping to the end on `break`, as the result of evaluating body will be in the accumulator,
    // and we do not need to enter the test or update blocks which could potentially change the accumulator.
    generator.begin_breakable_scope(Bytecode::Label { end_block }, label_set);

    // If the body contains no code, the result of the body will be undefined.
    generator.emit<Bytecode::Op::LoadImmediate>(js_undefined());
    TRY(m_body->generate_bytecode(generator));
    ...
    if (m_update) {
        ...
        // Store the result of executing the body into the result register.
        // This is done here because the body block may have been block terminated (e.g. via unconditional `continue`), meaning we can't generate the store in the body block.
        // Note that we only need to do this if there's a test block, as that is the only block that jumps to the `load_result_and_then_exit_block` block.
        // If there's no test block, we will instead jump to the body block here AFTER running the update block, meaning the accumulator will always contain the result of the body block.
        if (m_test)
            generator.emit<Bytecode::Op::Store>(result_reg);
    ...
    if (load_result_and_then_exit_block) {
        generator.switch_to_basic_block(*load_result_and_then_exit_block);
        generator.emit<Bytecode::Op::Load>(result_reg);
        generator.emit<Bytecode::Op::Jump>().set_targets(
            Bytecode::Label { end_block },
            {});
...
{
    bool previous_declarator_loaded_undefined = false;
    for (auto& declarator : m_declarations) {
        if (declarator.init()) {
            previous_declarator_loaded_undefined = false;
            TRY(declarator.init()->generate_bytecode(generator));
        } else if (!previous_declarator_loaded_undefined) {
            previous_declarator_loaded_undefined = true;
            generator.emit<Bytecode::Op::LoadImmediate>(js_undefined());
        }
...
Bytecode::CodeGenerationErrorOr<void> TryStatement::generate_bytecode(Bytecode::Generator& generator) const
{
    ...
    if (m_finalizer) {
        ...
        generator.start_boundary(Bytecode::Generator::BlockBoundaryType::ClearPendingException);
        TRY(m_finalizer->generate_bytecode(generator));
        generator.end_boundary(Bytecode::Generator::BlockBoundaryType::ClearPendingException);
        if (!generator.is_current_block_terminated()) {
            next_block = &generator.make_block();
            auto next_target = Bytecode::Label { *next_block };
            generator.emit<Bytecode::Op::GetPendingException>();
    ...
    // This boundary is to prevent us from leaving environments or other unwind contexts.
    // For example: f = () => { try { throw 1; } catch {} }
    // Without this boundary, the `throw` would leave the variable environment of the function,
    // which is incorrect because the environment can still be used in `catch` or after the try/catch block.
...
static Bytecode::CodeGenerationErrorOr<ForInOfHeadEvaluationResult> for_in_of_head_evaluation(Bytecode::Generator& generator, IterationKind iteration_kind, Variant<NonnullRefPtr<ASTNode>, NonnullRefPtr<BindingPattern>> const& lhs, NonnullRefPtr<ASTNode> const& rhs)
{
    ...
    if (auto* ast_ptr = lhs.get_pointer<NonnullRefPtr<ASTNode>>(); ast_ptr && (is<VariableDeclaration>(**ast_ptr) || is<Expression>(**ast_ptr))) {
        ...
        if (is<VariableDeclaration>(**ast_ptr)) {
...
Bytecode::CodeGenerationErrorOr<void> ForInStatement::generate_labelled_evaluation(Bytecode::Generator& generator, Vector<FlyString> const& label_set) const
{
    ...
    TRY(for_in_of_body_evaluation(generator, *this, m_lhs, body(), head_result, loop_end, loop_update));
    generator.end_boundary(Bytecode::Generator::BlockBoundaryType::BreakLabel);
    return {};
}
