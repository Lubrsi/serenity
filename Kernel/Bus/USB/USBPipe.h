...
public:
    static KResultOr<NonnullOwnPtr<Pipe>> try_create_pipe(NonnullRefPtr<USB::USBController> controller, Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, i8 device_address, u8 poll_interval = 0);
    ...
    Pipe(NonnullRefPtr<USBController> controller, Type type, Direction direction, u16 max_packet_size);
    Pipe(NonnullRefPtr<USBController> controller, Type type, Direction direction, USBEndpointDescriptor& endpoint);
    Pipe(NonnullRefPtr<USBController> controller, Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, u8 poll_interval, i8 device_address);
