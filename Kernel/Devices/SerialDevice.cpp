...
bool SerialDevice::can_read_without_blocking(const FileDescription&, size_t) const
{
...
bool SerialDevice::can_write_without_blocking(const FileDescription&, size_t) const
{
...
KResultOr<size_t> SerialDevice::write(FileDescription& description, u64, const UserOrKernelBuffer& buffer, size_t size)
{
    ...
    ScopedSpinLock lock(m_serial_lock);
    if (!can_write_without_blocking(description, size))
        return EAGAIN;
